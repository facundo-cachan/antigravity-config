...
- If mode is `engram`:
  ...
  **Run all retrieval calls in parallel** — call all mem_get_observation calls simultaneously in a single response.
  ...
  1. `mem_get_observation(id: {proposal_id})` → full proposal
  2. `mem_get_observation(id: {spec_id})` → full spec
  3. `mem_get_observation(id: {design_id})` → full design
  4. `mem_get_observation(id: {tasks_id})` → full tasks
